...
2. **A hardening pass on the HTTP inbound adapter** following CQS + Spring Modulith:
   ...
   - then re-packaged into **Hybrid CQS** — nested `Command.Result` (1-1 locality) and a dedicated
     `port.inbound.query.view` package for the multi-query `View` types.
...
- **Query side (multi-1, global):** `View` types live in the sub-package `port.inbound.query.view`
  (`RoomDetailView`, `RoomSummaryView`). The Query *records* (`GetRoomByIdQuery`, `GetRoomByNameQuery`)
  stay in `port.inbound.query`, independent and type-safe — because a single View can serve many queries and
...
- **Reference module:** Room now demonstrates the full golden recipe — one-way VOs, sealed domain events,
  CQS split, hybrid packaging, split inbound controllers, scoped advice. Other modules can copy it.
