...
- `docs/architecture/adr/0005-domain-factory-and-global-invariant.md` — **Accepted**: the global room-uniqueness
  invariant is a **Domain Policy** (`RoomUniquenessPolicy`, in `domain/model/policy/`). The aggregate receives the
  policy as an argument on every uniqueness-sensitive operation and throws `DuplicateRoomException` itself.
...
- A **set-based invariant** (e.g. no two rooms share the same `(building, floor, code)` coordinate, and no two
  ...
  to the **Domain** (Ubiquitous Language: `DuplicateRoomException` is domain vocabulary).
- Mechanism (**ADR 0005, Accepted**): a domain-owned `RoomUniquenessPolicy` interface in `domain/model/policy/`
  ...
  `DuplicateRoomException` with the correct `Reason`. There is **no** `RoomFactory` — the aggregate is the single
  decision-maker.
...
- The **DB unique constraints** (`uk_rooms_building_floor_code`, `uk_rooms_building_floor_name`) remain the
  ...
  `DuplicateRoomException` (correct `Reason`), so the TOCTOU race is still caught. The policy read and the DB
