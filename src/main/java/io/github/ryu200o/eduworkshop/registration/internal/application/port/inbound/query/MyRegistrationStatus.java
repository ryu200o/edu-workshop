 ...
 * {@code REFUNDED} is a system-initiated outcome (workshop cancelled), so it is deliberately not a
 * filter a learner may choose. The view enum still mirrors {@link RegistrationState} so that a
 * non-filtered query (Epic 2 sign-off Q4) can display the full history including {@code REFUNDED}.</p>
 */
