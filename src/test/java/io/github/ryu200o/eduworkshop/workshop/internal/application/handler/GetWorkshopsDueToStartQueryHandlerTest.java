...
class GetWorkshopsDueToStartQueryHandlerTest {

    @Mock
    private WorkshopReader workshopReader;

    private GetWorkshopsDueToStartQueryHandler handler() {
        return new GetWorkshopsDueToStartQueryHandler(workshopReader);
    }
    ...
    void returnsDueViewsFromPort() {
        ...
        when(workshopReader.getPublishedDueToStart(now)).thenReturn(List.of(one));

        List<WorkshopSummaryView> result = handler().handle(new GetWorkshopsDueToStartQuery(now));
    ...
    void returnsEmptyListWhenNoneDue() {
        Instant now = Instant.parse("2026-09-01T12:00:00Z");
        when(workshopReader.getPublishedDueToStart(now)).thenReturn(List.of());

        assertThat(handler().handle(new GetWorkshopsDueToStartQuery(now))).isEmpty();
    }
}